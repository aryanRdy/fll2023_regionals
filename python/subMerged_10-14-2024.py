...
async def run1():
   ...
   # await straight(300,200,1)#puts attachment on boat
    ...
    '''
    await turn(Direction.LEFT,from asyncio import sleep
from hub import port
from hub import motion_sensor
import motor
import runloop
...
async def run1():
   ...
   # await straight(300,200,1)#puts attachment on boat
    ...
    await straight(1000,250,1)# goes to collect third krill
    await straight(300,250,-1)#backs up from whale
    await turn(Direction.RIGHT,90,200)#turns to align with sonar discovery mission
    await straight(500,650,-1)#reaches sonar discovery
    await turn(Direction.LEFT,7,200)# aligns robot with sonar discovery
    await motor.run_for_degrees(port.D,2000,1000)# does sonar discovery
    await straight(500,150,-1)
    await turn(Direction.RIGHT,15,200)
    await straight(500,800,1)
    await turn(Direction.RIGHT,20,200)
    await straight(1050,1000,1)
    '''
    await turn(Direction.LEFT,35,100)# turns to align with second krill
...
35,100)# turns to align with second krill
    await straight(200,100,1)
    motor.run_for_degrees(port.F,-550,1000)# lifts boat mission attachment down
    await motor.run_for_degrees(port.D,-400,1000)
    await straight(200,100,-1)
